	...
	<div class="relative z-10 max-w-7xl mx-auto px-6 space-y-20 lg:space-y-28">
		<?php foreach ($structure as $node): ?>
			...
			<?php elseif ($type === 'split_panel'): ?>
				...
				?>
					<div class="grid lg:grid-cols-2 gap-px rounded-[2rem] overflow-hidden max-w-6xl mx-auto items-stretch" style="background: color-mix(in srgb, var(--primary) 14%, transparent);">
					<div class="min-w-0 p-8 lg:p-12 flex flex-col justify-center space-y-5 <?= $reverse ? 'lg:order-2' : ''; ?>" style="background: var(--card);">
						<?php if ($slug !== ''): ?>
						...
						<?php endif; ?>
						<h3 class="text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight" style="font-family: 'JetBrains Mono', monospace; color: var(--foreground); overflow-wrap: anywhere; word-break: normal; hyphens: auto;">
							<?= $escape($title); ?>
							<?php if ($accent !== ''): ?>
								<span style="color: rgb(0, 200, 255);"> <?= $escape($accent); ?></span>
							<?php endif; ?>
						...
						<?php if ($body !== ''): ?>
							<p class="text-base leading-relaxed" style="color: var(--muted-foreground); overflow-wrap: anywhere; word-break: normal; hyphens: auto;">
								<?= $escape($body); ?>
							</p>
						<?php endif; ?>
						<?php if (!empty($points)): ?>
