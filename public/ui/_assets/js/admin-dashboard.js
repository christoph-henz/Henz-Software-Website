...
(function () {
    ...
    var cfg = window.__ADMIN_DASHBOARD_CONFIG || {};
    var canViewBookings = !!cfg.can_view_bookings;
    var canViewKpi = !!cfg.can_view_kpi;
    var canManageBookings = !!cfg.can_manage_bookings;
    ...
    function statusClass(booking) {
        ...
        if (s === 'confirmed') return 'admin-calendar-event--confirmed';
        if (s === 'cancelled') return 'admin-calendar-event--cancelled';
    ...
    function fetchBookings() {
        ...
        var bookingsFetch = fetch((cfg.api && cfg.api.bookings ? cfg.api.bookings : '/v1/admin/bookings') + '?' + params.toString(), {
            credentials: 'include',
    ...
    function renderCalendar() {
        ...
        calendarRoot.innerHTML =
            ...
            (canManageBookings ? '      <button type="button" class="admin-calendar-btn" data-cal-create>Neue Buchung</button>' : '') +
            '    </div>' +
    ...
    function bookingsForDay(dateKey) {
        return state.bookings.filter(function (b) {
            ...
            var notCancelled = String(b.status || '') !== 'cancelled';
    ...
    function renderKpis() {
        ...
        var outstandingBookings = monthBookings.filter(function (b) {
            return String(b.payment_status || '') !== 'paid' && String(b.status || '') !== 'cancelled';
        });
        var cancelledBookings = monthBookings.filter(function (b) {
            return String(b.status || '') === 'cancelled';
        });
    ...
    function bindCalendarEvents() {
        ...
        calendarRoot.querySelectorAll('[data-slot-start]').forEach(function (el) {
            ...
            function openCreateForSlot() {
                if (!window.adminBookingsOpenCreateModal) {
                    return;
                }
                window.adminBookingsOpenCreateModal({ startedAt: slotStart });
            }
        ...
        if (create) {
            create.addEventListener('click', function () {
                if (window.adminBookingsOpenCreateModal) {
                    window.adminBookingsOpenCreateModal();
                }
            });
        }
    }
    ...
    function isRevenueEligibleBooking(booking) {
        ...
        var isCancelled = (status === 'cancelled' || status === 'storno');
        if (!isCancelled) {
            return false;
        }
